...
export class CanvasOverlay {
  ...
  private pageHeightPt = 841; // updated on each render
  private scale = 1.5;

  private handlers: AnnotationCreatedHandler[] = [];
  ...
  setStyle(style: ActiveToolState): void {
    ...
    this.canvas.style.pointerEvents =
      style.tool !== "select" && style.tool !== "signature" ? "auto" : "none";
  }

  setTool(tool: ToolKind): void {
    this.activeTool = tool;
    this.canvas.style.pointerEvents =
      tool !== "select" && tool !== "signature" ? "auto" : "none";
    if (tool === "text") this.canvas.style.cursor = "text";
    else if (tool !== "select") this.canvas.style.cursor = "crosshair";
  ...
  syncToPage(
    ...
    // Match overlay canvas to PDF canvas
  ...
  /** Redraw all committed annotations for the current page. */
  ...
  private drawAnnotation(ann: Annotation): void {
    ...
    if (ann.kind === "rect") {
      ...
      if (ann.underline) {
        ...
        ctx.lineWidth = 1;
        const startX =
          ann.alignment === "left" ? x : ann.alignment === "right" ? tx - w : tx - w / 2;
        ctx.beginPath();
  ...
  private onMouseDown = (e: MouseEvent): void => {
    if (this.activeTool === "select" || this.activeTool === "signature") return;
  ...
  private onMouseUp = (e: MouseEvent): void => {
    ...
    // Convert canvas bbox to PDF coordinates.
    // PDF Y origin is at bottom; canvas Y at top.
    // x,y in PDF = bottom-left corner of the bbox.
    const pdfTL = canvasToPdf(x0, y0, this.scale, this.pageHeightPt);
    const pdfBR = canvasToPdf(x0 + w, y0 + h, this.scale, this.pageHeightPt);
    const pdfX = pdfTL.x;
    const pdfY = pdfBR.y; // bottom of bbox in PDF coords
  ...
  private handleTextClick(canvasX: number, canvasY: number): void {
    ...
    const commit = (): void => {
      ...
      if (content) {
        // Estimate bounding width from rendered text
  ...
  placeSignature(
    ...
    const ann: SignatureAnnotation = {
      ...
      page: this.currentPage,
      x: pdfCoords.x,
      y: pdfCoords.y - heightPt,  // top-left click → bottom-left PDF coord
      width: widthPt,
      height: heightPt,
      imageData,
    };
